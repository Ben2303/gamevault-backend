...
//import sharp from "sharp"; // REMOVED FOR NON AVX CPU SUPPORT
...
export class ImagesService {
  ...
  private async downloadImage(sourceUrl: string): Promise<Image> {
    ...
    try {
      ...
      // Compress Image
      // this.logger.debug(`Compressing image from '${image.source}'...`); // REMOVED FOR NON AVX CPU SUPPORT
      // const compressedImageBuffer = await sharp(imageBuffer).toBuffer(); // REMOVED FOR NON AVX CPU SUPPORT

      // Save the Image
      this.logger.debug(`Saving image from '${image.source}'...`);
      if (!configuration.TESTING.MOCK_FILES) {
        // fs.writeFileSync(image.path, compressedImageBuffer); // REMOVED FOR NON AVX CPU SUPPORT
        fs.writeFileSync(image.path, imageBuffer); // ADDED FOR NON AVX CPU SUPPORT
        this.logger.debug(
